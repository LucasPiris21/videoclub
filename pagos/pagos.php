...
if (empty($_GET['submit']) or isset($_GET['clear'])) {
	...
	$busq=$_GET['busq'];
	$query="SELECT * FROM `pagos` WHERE concat(id_pago,monton,dni_personal,id_factura) LIKE '%$busq%'";
	$consulta=mysqli_query($conexion,$query);
}
			...
			<div class="col-md-10">
							...
							<?php while ($fila=mysqli_fetch_array($consulta)) {
								echo "<tr>";
									...
									echo "<td class='text-center'>";
										echo '<a href="deletepagos.php?id='.$fila[0].'&fact='.$fila[3].'" class="btn btn-danger">Eliminar</a>';
									echo "</td>";
									echo "<td class='text-center'>";
										echo '<a href="updatepagosform.php?id='.$fila[0].'" class="btn btn-danger">Modificar</a>';
									echo "</td>";
								echo "</tr>";
							 }?>
						</tbody>
				...
				<div class="row justify-content-star">
						<div class="col-md-auto align-self-center">
							<a href='altapagosform.php' class="add btn btn-danger">Agregar Pago</a>
						</div>
				</div>
			</div>
		</div>
