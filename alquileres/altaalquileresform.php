...
<div class="container-sm form main">
	...
	<div class="row justify-content-start">
		<div class="col-md-8">
				...
				<div class="form-group row">
					...
					<div class="col">
						...
						<?php
							...
							$query="SELECT `id_bono`, `credito_disponible`, `periodo`, `dni_cliente` FROM `bono` WHERE `fecha_vencimiento`>'$fecha_actual' and `credito_disponible`>'0'";
						...
						<input class="form-control" name="id_bono" list="id_bono" style="width:200px" value="<?php if (isset($_POST['bono'])) { $id_bono=$_POST['id_bono']; echo $id_bono; }?>" required>
					</div>
					<div class="col">
			...
			<?php
			if (isset($_POST['bono']) and $id_bono<>null) {
				$id_bono=$_POST['id_bono'];
				$query="SELECT `credito_disponible`, `periodo`, `dni_cliente` FROM `bono` WHERE `id_bono`='$id_bono' and `fecha_vencimiento`>'$fecha_actual'";
				$consulta=mysqli_query($conexion,$query);
				if (mysqli_num_rows($consulta)>0) {
						...
						for ($i=0; $i <$credito ; $i++) { 
							echo "<div class='form-group row'>";
								echo "<div class='col'>";
									...
									echo "</datalist>";
									echo "<input type='number' class='form-control' name='pelicula[]' list='pelicula' min='1' pattern='^[0-9]+''>";
								echo "</div>";
							echo "</div>";
						}
						echo "<div class='form-group row'>";
					...
					echo "No existe un bono disponible con el ID ingresado";
				}
			}
			?>
		</div>
